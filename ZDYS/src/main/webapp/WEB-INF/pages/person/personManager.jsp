...
function opentoadduser(){
	window.open ("<%=path%>/person/toAddPerson.do", "newwindow", "height=600, width=650, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,top=200,left=500");
}
...
$(function(){
        ...
        $("input[name='checkbox']:checked").each(function() { 
      		   window.open ("<%=path%>/person/toUpdatePerson.do?id="+$(this).val(), "newwindow", "height=600, width=600, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,top=200,left=500");
        });
...
<body>
    ...
    <div style="font-size: 25px">
     <form action="<%=path%>/person/getAllPerson.do" method="post" id="personmanager">
   		<span> 
	          姓名:<input type="text" name="xm" value="${xm}" />
	          身份证号:<input type="text" name="sfzh" value="${sfzh}" />
    ...
    <table class="tablelist"  id="table">
        ...
        <c:forEach items="${pages.root }" var="zperson">
        <tr>
        <td><input name="checkbox" type="checkbox" value="${zperson.id }" /></td>
        <td>${zperson.xm }</td>
        <td>${zperson.xb }</td>
        <td>${zperson.sfzh }</td>
        <td>${zperson.mz }</td>
